...
1. ✅ **공개 주소에서 합성 기록을 추가·수정·삭제할 수 있다**
   ...
   - 증거: 로컬 origin/main == HEAD(카드5 커밋 `3f9568c` 포함 전부 푸시됨), 위 카드1 스크린샷(추가·수정·삭제)
2. ✅ **고유 ID로 정확한 한 건만 수정·삭제된다**
   - 판단: 동일 항목 여러 건 중 대상 1건만 바뀌고 나머지는 그대로임을 확인
   - 증거: `Records.update`/`remove`는 항상 `id`로 대상 탐색 (위 카드1 스크린샷)
3. ✅ **필수값이 빈 기록은 저장되지 않는다**
   - 판단: 빈 값으로 저장 시도 시 거부됨
   - 증거: `validateRecordInput`(날짜·항목 필수), 위 검증 안내서 "안 될 때" 오류 문구 확인
4. ✅ **새로고침 뒤에도 기록 수와 값이 유지된다**
   - 판단: 새로고침 전·후 ID·날짜·값이 동일
   - 증거: 위 카드2 새로고침 전·후 스크린샷
5. ✅ **내보낸 파일에서 ID·날짜·값·단위가 복원된다**
   - 판단: 내보내기 → 가져오기 후 원본과 일치
   - 증거: `ImportExport.exportJSON`/`parseImport`, 위 카드3 v1 가져오기 결과
6. ✅ **전체 삭제 뒤 기록은 0건이다**
   - 판단: 전체 삭제 실행 후 목록 확인
   - 증거: `Records.removeAll()`
7. ✅ **손상 파일은 기존 기록을 바꾸지 않고 오류 이유를 표시한다**
   - 판단: 손상 파일 가져오기 시도 시 기존 기록 유지 + 오류 문구 표시
   - 증거: 위 카드2 손상 파일 거부 스크린샷
8. ✅ **v1·v2 변환 뒤 건수와 기존 값이 유지된다**
   - 판단: 변환 전·후 ID·날짜·값·단위 대조 일치
   - 증거: 위 카드3 스크린샷([fixtures/v1-sample.json](fixtures/v1-sample.json) 가져오기 결과)
9. ✅ **변환을 두 번 실행해도 기록이 늘거나 다시 바뀌지 않는다(멱등성)**
   - 판단: 동일 파일로 변환 함수 2회 실행 후 건수·값 비교
   - 증거: `migrateRecordToV2`는 이미 v2면 원본 그대로 반환, Playwright로 변환·재로드 2회 후 값 불변 확인([PROGRESS.md](PROGRESS.md) #14)
10. ✅ **주간 요약의 기간과 집계값이 보인다**
    - 판단: 화면에 기준 시간대·주 시작 요일에 맞는 기간과 합계가 표시
    - 증거: 위 카드4/카드5 요약 화면 스크린샷
11. ✅ **잘못된 날짜·값이 집계에서 제외된다**
    - 판단: 예상 결과표와 실제 집계 결과 일치
    - 증거: [PROGRESS.md](PROGRESS.md) 카드4 예상 결과표 대조, 위 카드4 스크린샷
12. ✅ **실제 개인 기록·개인정보·비밀값이 공개 화면·파일·제출 기록에 0건이다**
    - 판단: [PROGRESS.md](PROGRESS.md) "항상 지킬 것" 체크리스트 통과
    - 증거: 백엔드 없음(순수 프론트엔드, 비밀값 존재 자체가 불가), 이 문서 스크린샷 전부 합성 데이터·[card5-log.md](card5-log.md)는 날짜·완료 여부만 기록
